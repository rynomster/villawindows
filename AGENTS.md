...
- **GitHub Pages**: Deployed via GitHub Pages. The `baseurl` in `_config.yml` is set to `/villawindows`. Always use the `relative_url` filter for internal links and assets: `{{ '/path' | relative_url }}`.
