...
- **GitHub Pages**: Deployed via GitHub Pages. The `baseurl` in `_config.yml` is set to `""` (empty string) because the site uses a custom domain (`villawindows.lunatech.co.za`). Always use the `relative_url` filter for internal links and assets: `{{ '/path' | relative_url }}`.
